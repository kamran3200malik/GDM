...
watch(search, () => {
    router.get(route('admin.settings.units.index'), {
        search: search.value,
    }, {
        preserveState: true,
        replace: true,
...
const deleteUnit = (unit) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            deleteForm.delete(route('admin.settings.units.destroy', unit), {
                onSuccess: () => {
                    Swal.fire({
                        title: 'Deleted!',
...
const toggleStatus = (unit) => {
    router.post(route('admin.settings.units.toggle', unit));
};
...
const submitCreate = () => {
    createForm.post(route('admin.settings.units.store'), {
        onSuccess: () => {
            closeCreateModal();
            Swal.fire({
                title: 'Created!',
...
const submitEdit = () => {
    editForm.put(route('admin.settings.units.update', editingUnit.value), {
        onSuccess: () => {
            closeEditModal();
            Swal.fire({
                title: 'Updated!',
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex justify-between items-center">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-800">Units Management</h2>
                                <p class="text-gray-600 mt-1">Manage organizational units and departments</p>
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="unit in units.data" :key="unit.id">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 font-mono">{{ unit.code }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <button @click="toggleStatus(unit)" class="flex items-center">
                    ...
                    <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700" v-if="units.total > 0">
                                    Showing
                                    <span class="font-medium">{{ units.from || 0 }}</span>
                                    to
                                    <span class="font-medium">{{ units.to || 0 }}</span>
                                    of
                            ...
                            <div v-if="units.links && units.links.length > 0">
        ...
        <Modal :show="showCreateModal" @close="closeCreateModal" max-width="md">
            <div class="bg-white">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Create New Unit</h3>
                </div>
