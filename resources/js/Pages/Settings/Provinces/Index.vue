...
watch(search, () => {
    router.get(route('admin.settings.provinces.index'), {
        search: search.value,
    }, {
        preserveState: true,
        replace: true,
...
const deleteProvince = (province) => {
    Swal.fire({
        title: 'Are you sure?',
        text: `Are you sure you want to delete "${province.name}"?`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            deleteForm.delete(route('admin.settings.provinces.destroy', province), {
                onSuccess: () => {
                    Swal.fire({
                        title: 'Deleted!',
                        text: 'Province has been deleted successfully.',
                        icon: 'success',
                        timer: 2000,
...
const toggleStatus = (province) => {
    router.post(route('admin.settings.provinces.toggle', province));
};
...
const openEditModal = (province) => {
    ...
    editForm.name = province.name;
    editForm.code = province.code;
    editForm.is_active = province.is_active;
...
const submitCreate = () => {
    createForm.post(route('admin.settings.provinces.store'), {
        onSuccess: () => {
            closeCreateModal();
            Swal.fire({
                title: 'Created!',
                text: 'Province has been created successfully.',
                icon: 'success',
                timer: 2000,
                showConfirmButton: false
            });
...
const submitEdit = () => {
    editForm.put(route('admin.settings.provinces.update', editingProvince.value), {
        onSuccess: () => {
            closeEditModal();
            Swal.fire({
                title: 'Updated!',
                text: 'Province has been updated successfully.',
                icon: 'success',
                timer: 2000,
                showConfirmButton: false
            });
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex justify-between items-center">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-800">Provinces Management</h2>
                                <p class="text-gray-600 mt-1">Manage provinces and regions</p>
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="province in provinces.data" :key="province.id">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">{{ province.code || '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <button @click="toggleStatus(province)" class="flex items-center">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                                  :class="province.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                                                {{ province.is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </button>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <button @click="openEditModal(province)" 
                                        ...
                                        <button @click="deleteProvince(province)" 
                    ...
                    <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                        <div class="flex-1 flex justify-between sm:hidden">
                            <Link v-if="provinces.prev_page_url" :href="provinces.prev_page_url" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Previous
                            </Link>
                            <Link v-if="provinces.next_page_url" :href="provinces.next_page_url" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Next
                            </Link>
                        </div>
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700" v-if="provinces.total > 0">
                                    Showing
                                    <span class="font-medium">{{ provinces.from || 0 }}</span>
                                    to
                                    <span class="font-medium">{{ provinces.to || 0 }}</span>
                                    of
                                    <span class="font-medium">{{ provinces.total }}</span>
                                    results
                                </p>
                            ...
                            <div v-if="provinces.links && provinces.links.length > 0">
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    <template v-for="link in provinces.links" :key="link.label">
        ...
        <Modal :show="showCreateModal" @close="closeCreateModal" max-width="sm">
            <div class="bg-white">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Create New Province</h3>
                </div>
